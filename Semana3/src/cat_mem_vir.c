...
void catfd(int fdin, int fdout)
{
    ...
    if (fstat(fdin, &fdin_stat) == -1) //fstat da el tamaño del fichero en una struct stat que contiene el campo .st_size
    {
        perror("fstat(fdin)");
        exit(EXIT_FAILURE);
    }
    ...
    if (munmap(fdin_addr, fdin_stat.st_size) == -1) //para liberar memoria
    {
        perror("munmap(fdin)");
        exit(EXIT_FAILURE);
    }
